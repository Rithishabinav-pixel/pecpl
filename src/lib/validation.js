...
const CHAR_REGEX = /^[\p{L}\s]+$/u;

const SCRIPT_TAG_REGEX = /<\s*\/?\s*script\b[^>]*>/i;

function validateScriptTag(value) {
  if (SCRIPT_TAG_REGEX.test(value)) {
...
export function validateName(name) {
  ...
  const scriptError = validateScriptTag(value);
...
export function validateEnquiryForm({ name, phone, email, location,message }) {
  const errors = {
...
export function validateCareerForm({ name, phone, email, location,message }) {
  const errors = {
    name: validateName(name),
    phone: validatePhone(phone),
    email: validateEmail(email),
